...
function() {
	...
	function getSliceSHA1(opt) {
		var sha1Samples, defer = $.Deferred(),
		...
		return reader.onload = function(e) {
			if (file && !(file.length < 1)) {
				...
				len = read + unit > fileSize ? fileSize - read: unit,
				notEnd = read + len < fileSize,
				sha1 = notEnd ? middle: sha1Algo.getHash("HEX");
				if (!pushPartAndCheck({
					offset: read,
					datalen: len,
					datasha: sha1
				...
				notEnd ? readAsBinStr.call(reader, file.slice(read, Math.min(read + unit, fileSize))) : defer.resolve(uploadParts)
			}
		},
		...
		$.ajax({
			...
			success: function(res) {
				...
				0 == res.code) {
					...
					for (var sItem, sha1Samples = {},
					i = 1; i < opt.uploadparts.length; i *= 2) sItem = opt.uploadparts[i - 1],
	...
	function sliceUpload(opt) {
		...
		task = {
			...
			onTaskProgress: function(immediately) {
				var progress = function() {
					progressTimer = 0;
					for (var loaded = 0,
					i = 0; i < task.chunks.length; i++) {
						loaded += task.chunks[i].loaded
					}
					opt.onprogress && opt.onprogress(loaded / file.size, 1)
				};
				if (immediately) clearTimeout(progressTimer),
				progress();
				else {
		...
		function() {
			...
			if (opt.listparts) for (i = 0; i < opt.listparts.length; i++) partMap[opt.listparts[i].offset] = opt.listparts[i];
		...
		uploadNextChunk = function() {
			for (; that._uploadingThreadCount < that.uploadMaxThread && task.currentIndex < task.chunkCount; task.currentIndex++) {
				...
				function(chunk) {
					...
					uploadSingleChunk.call(that, task, chunk,
					function(error, data) {
						task.uploadingCount--,
						that._uploadingThreadCount--,
						error ? (chunk.state = "error", uploadError(0, data)) : (chunk.state = "success", task.uploadingCount <= 0 && task.currentIndex >= task.chunkCount ? uploadSuccess() : uploadNextChunk())
					})
				} (chunk)
			}
		};
	...
	function readAsBinStr(fileData) {
		var readFun;
		FileReader.prototype.readAsBinaryString ? readFun = FileReader.prototype.readAsBinaryString: FileReader.prototype.readAsArrayBuffer ? readFun = function(fileData) {
			var binary = "",
			pt = this,
			reader = new FileReader;
			reader.onload = function(e) {
				for (var bytes = new Uint8Array(reader.result), length = bytes.byteLength, i = 0; i < length; i++) binary += String.fromCharCode(bytes[i]);
				pt.content = binary,
				pt.onload()
			},
		...
		that.getAppSign(function(sign) {
			...
			sliceList.call(that, opt).always(function(res) {
				...
				if (data && data.session) {
					if (data.filesize !== opt.file.size) return error({
						code: -1,
	...
	var rememberSha1 = function(session, sha1Samples, sha1CacheExpired) {
		...
		for (var i = localStorage.length - 1; i >= 0; i--) {
			...
			current - localStorage.getItem(key).update_time > sha1CacheExpired && localStorage.removeItem(key)
		}
	...
	uploadSingleChunk = function(task, chunk, callback) {
		...
		blob = that.slice.call(file, offsetStart, offsetEnd),
		removeXhr = function(xhr) {
			for (var i = task.uploadingAjax.length - 1; i >= 0; i--) xhr === task.uploadingAjax[i] && task.uploadingAjax.splice(i, 1)
		},
		uploadChunk = function(cb) {
			...
			that.getAppSign(function(sign) {
				...
				ajax = $.ajax({
					...
					success: function(res) {
						res = res || {},
						0 === res.code ? cb(null, res) : cb("error", res)
					},
					error: function() {
						cb("error")
					},
		...
		tryUpload = function(times) {
			uploadChunk(function(err, data) {
				err ? times >= that.uploadMaxRetryTimes || task.uploadError ? callback(err, data) : tryUpload(times + 1) : callback(err, data)
			})
		};
		tryUpload(1)
	};
...
function() {
		...
		return opts.videoFile && (fileMessage.videoMsg = getFileMessage(opts.videoFile, opts.videoName), fileMessage.videoMsg.error) ? fileMessage.videoMsg.error: opts.coverFile && (fileMessage.coverMsg = getFileMessage(opts.coverFile, opts.coverName), fileMessage.coverMsg.error) ? fileMessage.coverMsg.error: void opts.getSignature(function(signature) {
			...
			$.ajax({
				...
				success: function(result, status, xhr) {
					...
					0 == result.code ? (opts.videoFile && setCookie(opts.videoFile.name, result.data.vodSessionKey, 24), uploadFile(opts, result, signature)) : ((result.code = 10005) && (opts.videoFile && delCookie(opts.videoFile.name), startUpload(opts)), result.code >= 2e4 && result.code <= 29999 && opts.retryStartNum > 0 && (opts.retryStartNum--, startUpload(opts)), opts.finish && opts.finish({
						message: result.message
					}))
					console.log(result.message);
				},
				error: function(xhr, status, error) {
					opts.retryStartNum > 0 && (opts.retryStartNum--, startUpload(opts)),
	...
	uploadFile = function(opts, result, signature) {
		if (result.data.video) {
			...
			}).uploadFile(successCallBack(opts, "video", result, signature), errorCallBack(opts, "video"), progressCallBack(opts, "video"), result.data.storageBucket, result.data.video.storagePath, opts.videoFile, 1)
	...
	finishUpload = function(opts, result, signature) {
		...
		$.ajax({
			...
			success: function(result, status, xhr) {
				...
				}) : (result.code >= 2e4 && result.code <= 29999 && opts.retryFinishNum > 0 && (opts.retryFinishNum--, finishUpload(opts, result, signature)), opts.finish && opts.finish({
					message: result.message
				}))
			},
			error: function(xhr, status, error) {
